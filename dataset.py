...
class TrainValidImageDataset(Dataset):
    ...
    def __init__(self, image_dir: str, lr_dir: str, image_size: int, upscale_factor: int, mode: str) -> None:
        super(TrainValidImageDataset, self).__init__()
        # Get all image file names in folder
        self.image_file_names = [os.path.join(image_dir, image_file_name) for image_file_name in os.listdir(image_dir)]
        # Get all lr file names in folder
        self.lr_file_names = [os.path.join(lr_dir, lr_file_name) for lr_file_name in os.listdir(lr_dir)]
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        if self.mode == "Train":
            lr_image = cv2.imread(self.lr_file_names[batch_index], cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.
            # bicubic
            lr_image = imgproc.imresize(lr_image, 1 / self.upscale_factor)
            # cv2.imshow("ll", lr_image)
            # cv2.waitKey(0)
            # cv2.imwrite('E:/learnable-downsample/data/NWPU VHR-10 dataset/bicubic/' + self.image_file_names[batch_index] + ".png", lr_image)
        else:
            lr_image = imgproc.imresize(hr_image, 1 / self.upscale_factor)
        ...
        return {"hr": lr_y_tensor, "lr": hr_y_tensor}
