...
def main():
    # Initialize training to generate network eva
    # luation indicators
    best_psnr = 0.0
    ...
    for epoch in range(config.start_epoch, config.epochs):
        ...
        # Automatically save the model with sthe highest index
        ...
        if epoch % 5 == 0:
            torch.save({"epoch": epoch,
                        ...
                        "state_dict": model.state_dict(),
                        "optimizer": optimizer.state_dict()},
                       os.path.join(samples_dir, f"epoch_{epoch}.pth"))
            if is_best:
                shutil.copyfile(os.path.join(samples_dir, f"epoch_{epoch}.pth"),
                                os.path.join(results_dir, f"best_epoch{epoch}_{'%4.2f' %psnr}.pth"))
                print("Best model changed to epoch %d" % epoch)
...
def load_dataset() -> [CUDAPrefetcher, CUDAPrefetcher, CUDAPrefetcher]:
    ...
    train_datasets = TrainValidImageDataset(config.train_image_dir, config.train_image_dir_lr,
                                            config.image_size, config.upscale_factor, "Train")
    valid_datasets = TrainValidImageDataset(config.valid_image_dir, config.train_image_dir_lr,
                                            config.image_size, config.upscale_factor, "Valid")
...
def build_model() -> nn.Module:
    model = FSRCNN(config.downscale_factor).to(config.device)
    model = MY_FSRCNN(config.downscale_factor, num_channel=1).to(config.device)
