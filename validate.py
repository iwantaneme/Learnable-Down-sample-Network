...
import config
import imgproc
from my_model import  MY_FSRCNN


def main() -> None:
    # Initialize the super-resolution model
    model = MY_FSRCNN(config.upscale_factor, config.num_channel).to(config.device)
    print("Build FSRCNN model successfully.")

    # Load the super-resolution model weights
    checkpoint = torch.load(config.model_path, map_location=config.device)
    model.load_state_dict(checkpoint["state_dict"])
    print(f"Load FSRCNN model weights `{os.path.abspath(config.model_path)}` successfully.")
    ...
    for index in range(total_files):
        ...
        # hr_image = cv2.imread(lr_image_path).astype(np.float32) / 255.0
        hr_image = imgproc.imresize(lr_image, 1 / config.upscale_factor)

        x, y = lr_image.shape[0:2]
        lr_image_resize = cv2.resize(src=lr_image, dsize=(int(y/4), int(x/4)), interpolation=cv2.INTER_CUBIC)
        cv2.imwrite(config.sr_dir + "bicubic/" + file_names[index], lr_image_resize * 255.0)


        # if lr image and hr image are same size
        # hr_image = imgproc.imresize(hr_image, 1 / config.upscale_factor)
        # cv2.imwrite(config.sr_dir + "resize/" + file_names[index], hr_image * 255.0)
        ...
        # lr_ycbcr_re_image = imgproc.bgr2ycbcr(lr_image_resize, use_y_channel=False)
